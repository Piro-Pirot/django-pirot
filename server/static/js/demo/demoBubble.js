...
const loadBubbles = async(page) => {
    let cookie = document.cookie;
    let csrfToken = cookie.substring(cookie.indexOf('=') + 1);
    const url = '/demo/load_bubbles_ajax/';
...
function isSameWithNext(thisBubble, nextBubble) {
    ...
    && thisBubble['name'] == nextBubble['name'])) {
...
function isSameWithPrev(prevBubble, thisBubble) {
    ...
    && prevBubble['name'] == thisBubble['name']) {
...
const loadBubblesResponse = async (ajaxBubbles) => {
    ...
    try {
        ...
        await createBubble(firstBubble, false, true);
        // 더이상 반복을 돌 필요가 없음
        return 0;
    }
    ...
    if(ajaxBubbles.length < 3) {
        ...
        const lastBubble = ajaxBubbles[ajaxBubbles.length - 1];
        await createBubble(lastBubble, false, true);
    }
}
...
async function createBubble(bubbleData, timeFlag, profileFlag) {
    ...
    if(bubbleData['is_notice'] === NOTICE) {
        ...
        const bubble_r = bubbleData['r'];
        const bubble_g = bubbleData['g'];
        const bubble_b = bubbleData['b'];
        profileImg.style.backgroundColor = 'rgb(' + bubble_r + ',' + bubble_g + ',' + bubble_b + ')';
        ...
        if(profileFlag && bubbleData['name'] !== NAME) {
